...
```{r}
#todo unsure about whether the auto.arima function should be run on a ts or just the residuals, CO.residuals.auto.two is the later of the two options
# auto.arima model
...
=======
# Multivariate Time Series
>>>>>>> a10e3dad640a27996d6bf5f7c87cccd930e58bee
...
for(p in 1:3) {
  for(q in 0:7) {
    ...
    AICmatrix1[p,q+1] <- varma.model$aic
  }
}
...
for(p in 4:7) {
  for(q in 0:8) {
    varma.model <- VARMACpp(allResiduals, p=p, q=q, include.mean=F)
    AICmatrix3[p,q+1] <- varma.model$aic
  }
}
...
for(p in 4:7) {
  for(q in 9:11) {
    varma.model <- VARMACpp(allResiduals, p=p, q=q, include.mean=F)
    AICmatrix4[p,q+1] <- varma.model$aic
  }
}
