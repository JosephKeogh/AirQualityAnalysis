...
```
The model that performs the best in diagnostics is the ARIMA(4,0,11) model. It has the most points with high p-values in the Ljung-Box statistic plot. Both of the other models see their p-values decrease sooner as the lag increases.
...
## Final Model
The final model includes a trend and seasonality, using 5 different periods (384, 192, 128, 96, and 76.8 days). The ARIMA model of the residuals is a 4,0,11 model.
...
The diagnostic plot for the model of the residuals does not have problems. The p-value for the Ljung-Box stastic is above the dashed line for 17 lags, meaning the model is adequate for up to 17 lags. Future work could focus on increasing the number of lags for which the model is considered adequate.
...
In our univariate models, the residuals for CO and NO2 were both modeled with an ARMA(4,0,11) model. There were autoregressive and moving average terms in both cases. Because of this, we will test models with p from 1 to 4 and q from 0 to 11, in order to determine the best VARMA model.
