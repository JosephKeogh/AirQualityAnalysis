...
```
Based on the periodogram, there are multiple possibilities for the period of a seasonal component. There are fewer spikes than the periodogram for CO. There are still spikes, though, each of which indicate a possible frequency to explain the seasonality of the data. We predict that if seasonality is significant in the model, it will be based on a complex wave.
...
# visualize the most influencial periods
plot(sorted.Ts.NO, xlim = c(1,20), main="Influencial Periods for NO2", xlab="Index", ylab="Period (Days)")
...
# average the periods that are clustered
...
legend(0, 350, legend = c("Top 10 Periods and Trend", "Averaged Periods and Trend"), col = c("red", "blue"),
       lwd = 1)
```
...
# create arima models based on the residuals
...
### Diagnostics
```{r}
# test the possible models of the residuals
...
# choose time duration for simulation
## 365 was choosen because we are modeling a year
n = 365

CO.uni.sim <- arima.sim(n = n, list(ar = c(CO.residuals$coef[1], 
                                             CO.residuals$coef[2]), 
...
plot(CO.uni.sim + CO.mean, main = "CO Univariate Simulation", ylab="CO Value", xlab="Time (days)")
```
...
plot(CO.ts, main = "Simulation from Univariate CO Model", col = "black", ylim = c(-1,12), ylab="CO Value", xlab="Time (days)")
lines(CO.uni.sim.ts, col = "red")
legend(0, 12.5, legend = c("Recorded Data", "Simulation"), col = c("black", "red"), lwd = 1)
```
...
par(mfrow = c(1,2))
pg.CO <- spec.pgram(CO.ts,spans=9,demean=T,log='no', main="Collected Data Periodogram")
pg.CO.uni.sim <- spec.pgram(CO.uni.sim.ts, spans = 9, demean = T, log = 'no', main="Simulation Periodogram")
par(mfrow = c(1,1))
```
...
# simulation
NO.uni.sim <- arima.sim(n = n, list(ar = c(NO.residuals$coef[1]), 
...
plot(NO.uni.sim + NO.mean, main = "NO2 Univariate Simulation", ylab="NO value", xlab="Time (days)")
```
This is the plot of the simulation.
...
```{r}
# plot simulated values with observations
plot(NO.ts, main = "Simulation from Univariate NO2 Model", col = "black", ylim = c(0,350), ylab="NO value", xlab="Time (days)")
lines(NO.uni.sim.ts, col = "red")
legend(0, 350, legend = c("Collected Data", "Simulation"), col = c("black", "red"), lwd = 1)
```
...
# compare periodogram of observations and periodogram of simulation
par(mfrow = c(1,2))
pg.NO <- spec.pgram(NO.ts,spans=9, demean=T, log='no', main="Collected NO Data Periodogram")
pg.NO.uni.sim <- spec.pgram(NO.uni.sim.ts, spans = 9, demean = T, log = 'no', main="Simulated NO Periodogram")
par(mfrow = c(1,1))
```
...
# choose duration of simulation
n= 365

multi.sim <- VARMAsim(n=n, phi = varma.model$Phi,
...
legend(0, 12.5, legend = c("Collected Data", "Simulation"), col = c("black", "red"), lwd = 1)
```
...
legend(0, 340, legend = c("Collected Data", "Simulation"), col = c("black", "red"), lwd = 1)
```
The simulation values appear to be similar to the time series data.
...
### Create forecasts for CO
```{r}
# create new time data frame
...
plot(ts(air.test$CO.GT.),type='o',ylim=c(0,10), ylab="CO Level", main="Predicted CO Level and Measured")
...
```{r}
# Plot actual values and predicted values
plot(ts(air.test$NO2.GT.),type='o',ylim=c(50,275), ylab="NO2 Value", xlab="Time (Days)", main="Simulated and Measured NO2 levels")
...
# CO forecasts
plot(ts(air.test$CO.GT.),type='o', ylim = c(0,10), xlab="Time (Days)", main="Simulated and Measured CO levels", ylab="CO Levels")
...
```{r}
#NO forecasts
plot(ts(air.test$NO2.GT.),type='o',ylim = c(50,275),, xlab="Time (Days)", main="Simulated and Measured NO2 levels", ylab="NO2 Levels")
