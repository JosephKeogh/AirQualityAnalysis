...
```{r}
#todo unsure about whether the auto.arima function should be run on a ts or just the residuals, CO.residuals.auto.two is the latter of the two options
# when you run the first one with approximation = FALSE added, then it is the same as doing the second one. In the code I took the first one from (in TS3) it does not have approximation = FALSE, but the code that the second one is based on (from TS4) does have it

# auto.arima model
CO.residuals.auto <- auto.arima(e.ts.CO)
CO.residuals.auto.two <- auto.arima(CO.lm$residuals, approximation = FALSE)

# summary
summary(CO.residuals.auto)
summary(CO.residuals.auto.two)
```
...
```
The model that performs the best in diagnostics is the ARIMA(5,0,2) model. It has the most points with high p-values in the Ljung-Box statistic plot. It can be considered adequate for 15 lags, though the points from 11-15 lags are close to the dashed line. Both of the other models see their p-values approach 0 around 5 lags.

### Choose Model for Residuals
```{r}
CO.residuals <- CO.residuals.auto
```
Even though the auto-generated ARIMA model has a higher AIC value than the other two models, it performs much better in the diagnostic plot. We can consider the model adequate for 15 lags, instead of only 5 lags for the other models. Based on the diagnostics, we decided to choose the ARIMA(5,0,2) model as the final model for the residuals.

## Final Model
The final model includes a trend and seasonality, using 5 different periods (384, 192, 128, 96, and 76.8 days). The ARIMA model of the residuals is a 5,0,2 model.

### Get predictions from the final model
```{r}
#todo I tried to fix this based on the new coefficients for the ARIMA model but the plot keeps giving me an error -Camryn
# update I think I fixed whatever problem I was having, my code is under the CO simulation part and I did not use this to generate my simulation

# change the name of the model of the residuals
...
# the next time variable- next 365 days
...
# view the prediction
plot(CO.residuals + CO.mean, type = 'l', col='cyan')
```
...
The diagnostic plot for the model of the residuals does not have problems. The p-value for the Ljung-Box stastic is above the dashed line for 15 of 20 lags, meaning the model is adequate for at least up to 15 lags. Future work could focus on increasing the number of lags for which the model is considered adequate.
...
After testing the possible values of p, d, and q that we identified, we also used the auto.arima function to generate another model.
```{r}
#todo maybe change this once we know if we do auto.arima on time series of residuals or residuals themselves, do we need approximation = FALSE? the result is the same either way in this case
...
In our univariate models, the residuals for CO were modeled with an ARIMA(5,0,2) model and the residuals for NO2 were modeled with an ARIMA(1,0,4) model. There were autoregressive and moving average terms in both cases. Since there were different p and q values used to model the residuals for each time series, it is difficult to use the ACF and PACF to determine values for p and q for the multivariate model. Because of this, we will test models with p from 1 to 5 and q from 0 to 4 to determine the best VARMA model.
...
# p from 1 to 5, q from 0 to 4
AICmatrix <- matrix(NA, 5, 5)
for(p in 1:5) {
  for(q in 0:4) {
...
# AICmatrix.sorted
```
The two models with the lowest AIC values are VARMA(2,3) and VARMA(1,1) with an AIC of 7.098223 and 7.103681, respectively.

### Diagnostics
```{r}
# model with lowest AIC
varma.model1 <- VARMACpp(allResiduals, p=2, q=3, include.mean=F)

# model with second lowest AIC
varma.model2 <- VARMACpp(allResiduals, p=1, q=1, include.mean=F)
...
```
The VARMA(2,3) model performs best in the diagnostics. The model is adequate for 11 lags, as the p-values for the Ljung-Box statistic are large for these points. The second model we tested had no Ljung-Box statistics at any lag with a p-value different from 0.
...
# varma.model <- VARMACpp(allResiduals, p=2, q=3, include.mean=F)
```
The final model was chosen as the VARMA(2,3) model, because it has the lowest AIC and performed the best in the diagnostic plot.
